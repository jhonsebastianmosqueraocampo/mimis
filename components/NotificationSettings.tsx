...
import { Button, Checkbox, Divider, List, Modal, Portal, RadioButton, Text, useTheme } from 'react-native-paper';
  ...
  return (
    <>
      <View style={{ paddingHorizontal: 16, paddingVertical: 8 }}>
        ...
        <RadioButton.Group onValueChange={handleChange} value={selectedSettingsValue}>
      ...
      <Portal>
        <Modal visible={visible} onDismiss={() => setVisible(false)} contentContainerStyle={styles.modal}>
          <Text style={styles.modalTitle}>Selecciona tus {pluralize(2, labelDescription)}</Text>
          <Text style={styles.modalSubtitle}>
            {selectedItems.length} {pluralize(selectedItems.length, labelDescription)}{' '}
          ...
          <ScrollView style={{ maxHeight: 240 }}>
            {list.map((item) => (
              <List.Item
                key={item.id}
                title={item.title}
                left={() => <Image source={{ uri: item.img }} style={styles.checkboxIcon} />}
                right={() => (
                  <Checkbox
                    status={selectedItems.some((t) => t.id === item.id) ? 'checked' : 'unchecked'}
          ...
          <Button mode="contained" onPress={() => setVisible(false)} style={styles.closeButton}>
            Cerrar
          </Button>
        </Modal>
      </Portal>
