...
import type { Coach, LeagueB, PlayerB, RootStackParamList, Team } from "@/types";
...
export default function Search() {
  ...
  /* 🧱 Grid generator */
  const renderGrid = (items: any[], renderItem: (item: any) => React.ReactNode) => {
    const rows = [];
  ...
  const renderPlayers = () =>
    renderGrid(players, (p) => (
      <Card key={p.playerId} style={styles.card} onPress={()=>handleNavigation('player', p.playerId)}>
  ...
  const renderCoaches = () =>
    renderGrid(coaches, (c) => (
      <Card key={c.coachId} style={styles.card} onPress={()=>handleNavigation('coach', c.coachId)}>
  ...
  const renderLeagues = () =>
    renderGrid(filteredLeagues, (l) => (
      <Card key={l.league.id} style={styles.card} onPress={()=>handleNavigation('tournament', l.league.id)}>
  ...
  const renderTeams = () =>
    renderGrid(teams, (t) => (
      <Card key={t.teamId} style={styles.card} onPress={()=>handleNavigation('team', t.teamId)}>
...
const styles = StyleSheet.create({
  ...
  name: { fontWeight: "bold", fontSize: 13, textAlign: "center", color: "#111" },
